...
def connect_to_server(server_name):
    ...
    # Asigno a la variable ip_address la direccion ip correspondiente al hostname alojado en la variable server_name. Logro esto con la funcion propia del objeto socket. 

    # Aqui deberian obtener la direccion ip del servidor y asignarla a ip_address
    ...
    # Se crea un socket con la funcion definida para ello. Pide al menos dos argumentos que corresponden a un addr_family y type, respectivamente. El primero es un protocolo de internet (en este caso una direccion IP) y el segundo el tipo de transporte de datos (en este caso TCP) 

    # Aqui deben conectarse al puerto correcto del servidor
    new.connect((ip_address, HTTP_PORT))
    # Por ultimo conectamos el socket creado a la IP buscada anteriormente y al puerto 80 dado por HTTP_PORT. 
